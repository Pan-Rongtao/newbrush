...
public:
	...
	//获取行数，如果文件未打开，将抛出异常
	UInt64 GetLines() const;

	//读取以nBeg开始的nCount个字节，实际读到的字节数以返回的Stream的长度为准，如果文件未打开，将抛出异常
	Stream ReadBytes(UInt64 nBeg, UInt64 nCount) const;

	//读取第nLineIndex行的数据，如果文件未打开，将抛出异常
	Stream ReadLine(UInt64 nLineIndex) const;
	//ReadLines();
	...
	void WriteBytes(UInt64 nIndex, char *pData, UInt64 nSize);
	void WriteBytes(UInt64 nIndex, Stream ba);

	//在文件头部插入数据，如果文件未打开，将抛出异常
	void PrependBytes(UInt64 nIndex, char *pData, UInt64 nSize);
	void PrependBytes(UInt64 nIndex, Stream ba);

	//在文件尾部插入数据，如果文件未打开，将抛出异常
	void AppendBytes(UInt64 nIndex, char *pData, UInt64 nSize);
	void AppendBytes(UInt64 nIndex, Stream ba);

	//写入一行，会在需要写入的数据加上一个换行标志，如果文件未打开，将抛出异常
	void WriteLine(int nLineIndex, char *pData, UInt64 nSize);
