...
{
	auto target = std::make_shared<RttrObject>();
	//std::shared_ptr<RttrObject> target = nullptr;
...
TEST_CASE("test ColorAnimation", "[ColorAnimation]")
{
	...
	animation.Process += [target](const EventArgs &args) {
		...
		Log::info("animation procesing: time[{}], progress[{}], offset[{}, {}, {}]", time.toString().data(), progress, color.red(), color.green(), color.blue());
	};
	animation.Completed += [](const EventArgs &args) {
		Log::info("animation complete");
	};
...
TEST_CASE("test ColorAnimationUsingKeyFrames", "[ColorAnimationUsingKeyFrames]")
{
	...
	animation.Process += [target](const EventArgs &args) {
		...
		Log::info("animation procesing: time[{}], progress[{}], color[{}, {}, {}]", time.toString().data(), progress, v.red(), v.green(), v.blue());
	};
	animation.Completed += [](const EventArgs &args) {
		Log::info("animation complete");
	};
