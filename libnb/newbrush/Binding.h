...
class NB_API DataContext : public Object
{
public:
...
protected:
	DataContext(const std::string &_name) : name(_name), parent(nullptr) {}
...
{
public:
	DataObject(const std::string &name) : DataContext(name) {}
...
{
public:
	DataVar(const std::string &name, const T &v = T()) : DataContext(name) , m_v(v) {}
...
template<class T>
static ref<DataVar<T>> makeDataVar(const std::string &name, const T &value = T())						{ return createRef<DataVar<T>>(name, value); }
static ref<DataBool> makeDataBool(const std::string &name, bool v = false)								{ return makeDataVar<bool>(name, v); }
static ref<DataInt> makeDataInt(const std::string &name, int32_t v = 0)									{ return makeDataVar<int>(name, v); }
static ref<DataFloat> makeDataFloat(const std::string &name, float v = 0.0f)							{ return makeDataVar<float>(name, v); }
static ref<DataDouble> makeDataDouble(const std::string &name, double v = 0.0)							{ return makeDataVar<double>(name, v); }
static ref<DataString> makeDataString(const std::string &name, const std::string &v = std::string())	{ return makeDataVar<std::string>(name, v); }
static ref<DataObject> makeDataObject(const std::string &name)											{ return createRef<DataObject>(name); }
static ref<DataArray> makeDataArray(const std::string &name)											{ return createRef<DataArray>(name); }
static ref<DataVar<Color>> makeDataColor(const std::string &name, const Color &v = Colors::white)		{ return makeDataVar<Color>(name, v); }
static ref<DataVar<glm::vec2>> makeDataVec2(const std::string &name, const glm::vec2 &v = glm::vec2())	{ return makeDataVar<glm::vec2>(name, v); }
