...
void NBBParser::parse(const std::string & path, ref<Scene> &scene)
{
	auto result = m_parser.load(path);
	nbThrowExceptionIf(result.second, std::runtime_error, "parsing[%s] fail, error=[%d], error_message=[%s]", path.data(), result.second, error_message(result.second));
	...
	scene = variant_cast<ref<Scene>>(value);
...
void NBBParser::loopVisual(const dom::object &jObj, var &value, std::string &jPath)
{
	...
	else
	{
		...
		nbThrowExceptionIf(!classType, std::runtime_error, "[%s.Type]<%s> is unregistered", jPath.data(), classTypeName.data());
	...
	for (auto const &e : jObj)
	{
		...
		else
		{
			...
			nbThrowExceptionIf(!propValue, std::runtime_error, "invalid property type for [%s]", jPropName.data());
		}
...
var NBBParser::jsonToVar(const dom::element & json)
{
	...
	case dom::element_type::STRING:	v = std::string(json.get_string().first.data());		break;
	default: break;
	}
	return v;
}
