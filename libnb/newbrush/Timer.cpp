...
bool m_onDispatching = false;					//标记正在dispatch期间
std::set<Timer *> m_timerRemovedOnDispatching;	//记录在dispatch期间调用stop()/remove()的timer
std::multimap<uint64_t, Timer *> m_tickSequence;//tick队列
